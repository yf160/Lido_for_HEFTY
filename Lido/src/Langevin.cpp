...
double qhat_L_small_angle_LOpQCD(int pid, double E, double M, double T){
    ...
    return alpha_s(Q2cut, T) * CR * T * .5*mD2 * std::log(1+Q2cut/mD2)
         + CR * deltaQhat(T, M, E)/2.;
}


double qhat(int pid, double E, double M, double T){
double p0=std::sqrt(E*E-M*M);
if(std::abs(pid)==4) 
{ return charm_diffusion_table->get_qhat(p0,T);
} 
else if(std::abs(pid)==5) 
     {return bottom_diffusion_table->get_qhat(p0,T);
     }
 else 
...
double p0=std::sqrt(E*E-M*M);
if(std::abs(pid)==4) 
{ return charm_diffusion_table->get_qhat(p0,T)/2.;
} 
else if(std::abs(pid)==5) 
     {return bottom_diffusion_table->get_qhat(p0,T)/2.;
     }
 else 
