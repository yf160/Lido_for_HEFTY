...
diffusion_table  * charm_diffusion_table, * bottom_diffusion_table;
...
void initialize_mD_and_scale(int _mD_type, double _mu, double _afix, double _theta, double _cut){
	...
	charm_diffusion_table= new diffusion_table();
	bottom_diffusion_table= new diffusion_table();
...
:TL(0.8*0.196),TH(3.1*0.196),NT(24),dT((TH-TL)/(NT-1)),pL(0.0),pH(30.0),Np(151),dp((pH-pL)/(Np-1)) //Tc=0.196GeV
{ 

	qhat_tab.resize(Np);
        for(auto & it : qhat_tab) it.resize(NT);
}


/*void diffusion_table::read(std::string filename)
{
   ...
   int i=0;
   double qhat_tab[151][24];
//   std::cout<< " check" <<" " << i << std::endl;
   while(i < 151 && !file.eof()){
//        std::cout<< "check2" << 24 << " " << 151 << std::endl;
        for (int j=0; j<24; j++) 
        {
            file >>  qhat_tab[i][j];
            std::cout << i << " " << j << " " << qhat_tab[i][j] << std::endl;
        }
        i++;
 //       std::cout << i << " " << std::endl;
    }
...
while(! file.eof()){
        std::cout<< "check2" << NT << " " << Np << std::endl;
	for (int j=0; j<NT; j++) 
	{
	std::cout<< i << " " << j <<std::endl;
		file >> qhat_tab[i][j];
		std::cout << qhat_tab[i][j] << " " << std::endl;
	}
             i++;
}
}
...
double diffusion_table::get_qhat(double p, double T)
{return 0;
if (p<pL || p>pH || T<TL || T>TH)
	return 0;
    int iT = floor((T-TL)/dT);
    double rT=(T-TL-iT*dT)/dT;

    int ip = floor((p-pL)/dp);
    double rp=(p-pL-ip*dp)/dp;

    double wT[2];wT[0]=1-rT; wT[1]=rT;
    double wp[2];wp[0]=1-rp; wp[1]=rp;

    double result = 0.;
    for (int i=0; i<2; i++){ 
       for (int j=0; j<2; j++){
        result += qhat_tab[iT+i][ip+j]*wT[i]*wp[j];
    }
}
return result;
}
